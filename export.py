...
def fp16_write_model_v1(model, filename):
    print(f"write model to {filename}, the keys of model is {len(model.state_dict())}")
    config = model.config
    with open(filename, "wb") as file:
        head_len = 0
...
if __name__ == "__main__":
    ...
    python export.py --filepath="qwen1.5-0.5B_v1.bin" --dtype="fp16_v1" --model_type=Qwen/Qwen1.5-0.5B-Chat
    ...
    parser.add_argument("--dtype", choices=("fp32", "bf16", "fp16", "fp16_v1", "q80", "q40"), type=str, default="fp16")
    ...
    if (dtype == "fp16_v1"):
        fp16_write_model_v1(model, filepath)
    if (dtype == "bf16"):
