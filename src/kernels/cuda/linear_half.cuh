...
void linear_fwd_launch1(cublasHandle_t* handle, half* output, half* input, half *weight, half* bias, int batch, int in_features, int out_features) {
    int M = batch;
    int N = out_features;
    int K = in_features;
...
//     }
    ...
    for (int b = 0; b < M; b++) {
        printf("[");
        for (int i = 0; i < N; i++) {
            printf("%d=%f, ", i, __half2float(H_C[b * N + i]));
        }
        printf("]\n");
    }
#endif // LINEAR_DEBUG
