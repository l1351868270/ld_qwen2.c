...
void linear_forward(float* output, half* input, half *weight, half* bias, int batch, int in_features, int out_features) {
    int M = batch;
    int N = out_features;
    int K = in_features;


    gemv::gemv_fast_fwd(output, input, weight, M, N, K);
...
void* qwen2_forward(Context *ctx, Qwen2* qwen2, int *token, int batch, int pos) {
    ...
    for(int l = 0; l < p->num_hidden_layers; l++) {
        ...
        linear_forward(s->q, s->half_xb, w->q_proj_w + l * hidden_size * (num_heads * head_dim), w->q_proj_b + l * (num_heads * head_dim), batch, hidden_size, num_heads * head_dim);
        linear_forward(s->k, s->half_xb, w->k_proj_w + l * hidden_size * (num_key_value_heads * head_dim), w->k_proj_b + l * (num_key_value_heads * head_dim), batch, hidden_size, num_key_value_heads * head_dim);
        linear_forward(s->v, s->half_xb, w->v_proj_w + l * hidden_size * (num_key_value_heads * head_dim), w->v_proj_b + l * (num_key_value_heads * head_dim), batch, hidden_size, num_key_value_heads * head_dim);
        ...
        linear_forward(s->xb2, s->half_xb, w->o_proj + l * (num_heads * head_dim) * hidden_size, NULL, batch, num_heads * head_dim, hidden_size);
        ...
        float2half_forward<<<hidden_size / WARPGROUP_THREADS, WARPGROUP_THREADS>>>(s->half_xb, s->xb);
        linear_forward(s->hb, s->half_xb, w->gate_proj + l*intermediate_size*hidden_size, NULL, batch, hidden_size, intermediate_size);
        linear_forward(s->hb2, s->half_xb, w->up_proj + l*intermediate_size*hidden_size, NULL, batch, hidden_size, intermediate_size);
        ...
        linear_forward(s->xb, s->half_hb, w->down_proj + l* hidden_size * intermediate_size, NULL, batch, intermediate_size, hidden_size);
    ...
    linear_forward(s->logits, s->half_x, w->lm_head, NULL, batch, hidden_size, vocab_size);
