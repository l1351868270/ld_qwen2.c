...
def fp32_write_model(model, filename, mpi_np: int):
    print(f"write model to {filename}, the keys of model is {len(model.state_dict())}")
    config = model.config
    with open(filename, "wb"), open(filename, "wb") as file:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--mpi_np", type=int, default=2)
    ...
    if (dtype == "fp32"):
        fp32_write_model(model, filepath, mpi_np)
