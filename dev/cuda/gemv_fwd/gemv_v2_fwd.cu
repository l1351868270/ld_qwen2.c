...
__global__ void gemv_fp16_v1(size_t m, size_t n, size_t k,
...
void launch_gemm(size_t m, size_t n, size_t k, F const* alpha,
    ...
    const dim3 block_dim{WARP_THREADS, WARPGROUP_WARPS};
    unsigned int grid = (n + WARPGROUP_WARPS - 1) / WARPGROUP_WARPS;
    gemv_fp16_v1<<<dim3(1, grid), block_dim>>>(
        m, n, k, A, B, C);
    cudaDeviceSynchronize();

}

void gemv_v0_fwd(size_t m, size_t n, size_t k, half* A, half* B, float* C) {
